...
contract Pub{
    ...
    event published(bytes32 data, uint indexed stream_id, address indexed pub_id);
    ...
    function pub_to_event(bytes32 data, uint stream_id, address pub_id) public{
        ...
        emit published(data, stream_id,pub_id);
    ...
    function get_publisher(address _id) public view returns(address, uint[] memory){
        require(publisher[_id].exist == true, "publisher does not exist");
        return (publisher[_id].address_publisher, publisher[_id].access);
    }
